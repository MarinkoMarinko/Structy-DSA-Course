...
# TC: O(n)
# SC: O(k), where n = len(s) and k is the number of distinct chars (input)

def count_substring_exactly_k_distinct(s, k):      
    return count_substring_at_most_k_distinct(s, k) -  count_substring_at_most_k_distinct(s, k - 1)
